...
public class BookKeeperService {
	...
	public List<BookDetails> getAllBookRec(HttpServletRequest req) {
		String username = getUsername(req);
		RestTemplate restTemplate = new RestTemplate();
		ResponseEntity<String> bdd = restTemplate.getForEntity("http://localhost:8766/getRec?user=" + username, String.class);
	...
	public void saveBookRec(BookDetails bdd, HttpServletRequest req) {
		String username = getUsername(req);
		RestTemplate restTemplate = new RestTemplate();
		restTemplate.postForEntity("http://localhost:8766/saveRec?user=" + username, bdd, String.class);
	}
}